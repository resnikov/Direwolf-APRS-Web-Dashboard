...
if(isset($_GET['ajax'])) {
	...
	for ($n=0; $n<ceil(count($inp_stations_arr)/20);$n++) {
		$inp_chunk=array_slice($inp_stations_arr,20*$n,20); // aprs.fi accepts up to 20 names per request
		...
		foreach ( $inp_chunk as $inp_station ) { // for each station of this request, as defined in $stationsquery in config.php
